...
SW_JAR_FILE="$HOME/.apm/apache-skywalking-apm-bin/agent/skywalking-agent.jar"
SW_CONFIG_FILE="$HOME/.apm/apm.conf"
...
function start()
{
  ...
  CMD="nohup java $JVM_OPTS $SW_OPTS $CONFIG_OPTS $LOGGING_OPTS -jar $JAR_FILE  >/dev/null 2>&1 &"
...
## 获取skywalking的配置项构成Skywalking选项
function get_sw_opts()
{
  # 检测是否存在skywalking配置文件
  if [[ ! -f "$SW_CONFIG_FILE" ]];then
    warn "Can not find the sw config file: $SW_CONFIG_FILE, please run deploy_apm.sh first."
    return 1
  fi

  # 检测是否存在skywalking jar文件
  if [[ ! -f "$SW_JAR_FILE" ]];then
    warn "Can not find the skywalking agent: $SW_JAR_FILE, please run deploy_apm.sh first."
    return 1
  fi

  # 配置文件中保存的oap services的键值对
  key="SW_AGENT_COLLECTOR_BACKEND_SERVICES"
  value=`cat "$SW_CONFIG_FILE" | grep -m 1 "^\s*$key" | awk -F'=' '{ print $2 }' | sed s/[[:space:]]//g`

  if [[ -n "$value" ]]; then
    log "The url of skywalking server is $value defined in the file: $SW_CONFIG_FILE"
  else
    warn "Can not find the $key property in the skywalking config file: $SW_CONFIG_FILE."
    return 1
  fi

  SW_OPTS="-javaagent:$SW_JAR_FILE -Dskywalking.agent.service_name=$APP_NAME -Dskywalking.collector.backend_service=$value"
